...
const webhook = `WEBHOOK TO ANNOUNCE SNIPES HERE`;
const privateWebhook = `PUT YOUR WEBHOOK WHERE YOU WANT THE EMAIL AND PASSWORD TO BE SENT TO UPON SUCCESSFUL SNIPE`;

// https://pastebin.com/w5tFw41B
const announcementURL = `https://pastebin.com/raw/w5tFw41B`;
const version = '1.0.0';
const user = `DoozleDog`

const debug = false;

const prefix = `${`[`.cyan}${user.green}${`@`.cyan}${`Clover`.green}${`]`.cyan}`;
const suffix = `${`->`.cyan}`;
...
class Account {
        ...
        await fetch(`https://authserver.mojang.com/authenticate`, {
            ...
            if(response.status == 200){
                ...
                if(await this.needsSecurityQuestions()){
                    if(!await this.sendSecurityQuestions()){
                        console.log(`Invalid Security Questions were provided for ${this.email}`);
                    }
                } 
    ...
    async changeSkin(){
        await fetch(`https://api.minecraftservices.com/minecraft/profile/skins`, {
            ...
            body: JSON.stringify({
                url: `https://i.imgur.com/DQnERa0.png`,
                variant: `slim`
            }),
